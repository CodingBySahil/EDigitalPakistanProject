  ...
  return (
    <div className={`mx-auto flex min-h-screen w-full flex-col items-center justify-center bg-white font-poppins lg:flex-row ${className}`}>
      <div className="w-full md:w-1/2 lg:w-2/5">
        <img className="h-auto max-h-[400px] w-full rounded-[28px] object-cover md:max-h-[500px] md:rounded-[20px] lg:max-h-[600px] lg:rounded-[28px]" src={loginImg} alt="Login" />
      </div>

      <div className="flex w-full flex-col items-center justify-center p-6 lg:w-2/5 lg:p-6">
        <div className="w-full max-w-md">
          <h1 className="lg:text-4xl mb-6 text-center text-3xl font-bold text-black lg:text-left">Login to Digital Accelerated Learning Pathways</h1>

          <div className="mb-6 rounded-14xl bg-lightseagreen-200 p-4">
            <div className="flex justify-between">
              <Link to="/login" className="text-lg cursor-pointer rounded-14xl px-6 py-2 text-white no-underline bg-lightseagreen-200 hover:bg-lightseagreen-100">Login</Link>
              <Link to="/register" className="text-lg cursor-pointer rounded-14xl px-6 py-2 text-white no-underline hover:bg-lightseagreen-200">Register</Link>
            </div>
          </div>
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="text-lg w-full cursor-pointer rounded-xl bg-lightseagreen-100 py-3" disabled={loading}>{loading ? "Logging in..." : "Login"}</button>
          </form>

          {message && <p className={`text-lg mt-4 text-center ${loginSuccess ? "text-green-600" : "text-red-600"}`}>{message}</p>}
        </div>
      </div>
    </div>
  );
