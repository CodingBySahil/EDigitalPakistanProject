...
const MCQForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://your-api-endpoint.com/api/mcqs/batch', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg max-w-4xl mx-auto mt-8">
      <h2 className="text-3xl font-bold mb-6 text-center text-gray-800">Add Multiple MCQs</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block font-semibold text-lg mb-2">Chapter Code (ChapCode)</label>
          <select
            value={chapCode}
            ...
            required
          >
            <option value="" disabled>Select chapter code</option>
                ...
                {mcq.options.map((option, optIndex) => (
                  <div key={optIndex} className="flex items-center">
                    ...
                    <input
                      type="text"
                      value={option}
                      onChange={(e) => handleMCQChange(index, `option${optIndex + 1}`, e.target.value)}
